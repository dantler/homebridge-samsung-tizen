...
module.exports = class Remote {
    ...
    async getArtMode() {
        ...
        return Boolean(this.art && this.art.getStatus());
    }
    ...
    async setArtMode(value) {
        ...
        const retry = this.device.config.art_retry;

        let _do = () => this._setArtModeWithFallback(value, retry);
        ...
        if (!await this.getPower()) {
            ...
            // Wait for the art-app socket to be ready before issuing the command,
            // otherwise the panel silently drops it and ends up fully on.
            if (retry !== false) {
                await this.art.waitReady(retry.wake_timeout);

                if (retry.settle) {
                    await utils.delay(retry.settle);
                }
            }
    ...
    async _setArtModeWithFallback(value, retry) {
        // Legacy fire-and-forget behaviour when verification is disabled.
        if (retry === false) {
            return this.art.setStatus(value);
        }

        let confirmed = await this.art.setStatusVerified(value, retry);

        if (confirmed || !value || retry.fallback_power === false) {
            return confirmed;
        }

        this.device.log.debug('Art verify: falling back to short KEY_POWER click');

        try {
            await utils.race(this.api.click('KEY_POWER'), 2500);
        } catch (error) {
            this.device.log.debug(`Art verify: KEY_POWER fallback failed: ${error.message}`);
        }

        if (retry.fallback_settle) {
            await utils.delay(retry.fallback_settle);
        }

        let fallbackConfirmed = await this.art._probeStatus();

        if (fallbackConfirmed === true) {
            this.device.log.debug('Art verify: confirmed on after KEY_POWER fallback');
            return true;
        }

        this.device.log.debug(`Art verify: KEY_POWER fallback was not confirmed (got ${fallbackConfirmed})`);
        return false;
    }
