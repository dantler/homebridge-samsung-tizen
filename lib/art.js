...
module.exports = class Art {
    ...
    /**
     * Ask the TV for its current art-mode status and resolve once the TV has
     * reported a fresh status. Returns null when the TV did not respond, so
     * verification cannot be satisfied by the optimistic local status set by
     * setStatus().
    ...
    async _probeStatus() {
        ...
        let previousReportedStatusSequence = this.reportedStatusSequence;
        ...
        if (this.reportedStatusSequence === previousReportedStatusSequence) {
            return null;
        }
            ...
            .on('message', response => {
                ...
                if (response.event === 'ms.channel.connect') {
                    ...
                    readyFallback = setTimeout(() => {
                        this.device.log.debug('Art socket connected without ready event; treating connection as ready');
                        done();
                    }, 250);
