...
module.exports = class Device extends EventEmitter {
    constructor(config, platform, homebridge) {
        ...
        // Normalise Art Mode verify/retry settings (configurable)
        // art_retry: false to disable verification entirely, or an object:
        //   { attempts, interval, wake_timeout, settle }
        this.config.art_retry = require('./options/art_retry')(this.config);
    ...
    initEvents() {
        ...
        this.on('paired', () => {
            ...
            if (intervals.main) {
                setInterval(() => this.accessories[0].services.main.refreshValue(), intervals.main);
            }

            if (intervals.switch && this.accessories.length > 1) {
                setInterval(() => this.accessories.slice(1).forEach(element => element.services.main && element.services.main.refreshValue()), intervals.switch);
            }
        ...
        this.on('state.change', () => {
            this.accessories.forEach(element => element.services.main && element.services.main.refreshValue())
        });
    }
